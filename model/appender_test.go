...
func TestAppenderNoop(t *testing.T) {
	...
	minimal, err := os.ReadFile("./testdata/minimal.pdf")
	require.NoError(t, err)
	out, err := os.ReadFile(tempFile("appender_noop.pdf"))
	require.NoError(t, err)
...
func TestAppenderSignMultiple(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		pfxData, _ := os.ReadFile("./testdata/JohnSmith.pfx")
...
func TestSignatureAppearance(t *testing.T) {
	...
	pfxData, _ := os.ReadFile(testPKS12Key)
...
func TestAppenderExternalSignature(t *testing.T) {
	...
	getExternalSignature := func() ([]byte, error) {
		certFile, err := os.ReadFile(testPKS12Key)
		if err != nil {
			return nil, err
		}
