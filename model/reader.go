...
import (
	...
	"github.com/carmel/unipdf/core/security"
)
...
// NewPdfReaderFromFile creates a PdfReader from fp(pdf path)
func NewPdfReaderFromFile(fp string) (*os.File, *PdfReader, error) {
	fl, _ := os.Open(fp)
	reader, err := NewPdfReader(fl)
	return fl, reader, err
}
